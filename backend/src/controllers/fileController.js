...
const startMultipartUpload = asyncHandler(async (req, res) => {
    ...
    const {uploadId, key } = await fileServ.startMultiPartUpload({fileType:fileType});

    return res
            .status(200)
            .json(new ApiResponse(200, "multi-part uploading started", {uploadId, key}));
});
...
const generatePreSignedUrl = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, "pre-signed url generated successfully!!", {url, k})
    );
});
